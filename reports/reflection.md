...
| --- | --- | --- | --- |
| Sentinel nulls (`"N/A"`, whitespace) | 442 values, 5 columns | Counted as missing at profiling | Survives `dropna()`; corrupts every mean and join |
| Non-standard phone formats | 8 shapes, 1,026 rows | Reformatted; 34 quarantined | One customer looks like several; dedup fails |
| Unparseable / non-standard dates | 789 values | 750 repaired, 39 quarantined | Age and tenure wrong or crashing |
...
`income` shows why the first matters: 83 true nulls and 156 sentinels.
`dropna()` removes the 83 and leaves the 156 poisoning every downstream
...
That cost specificity — 25,373 raw matches reduced to 18,876, a **74.4%
...
Before masking, **100% of 3,684 cleaned records were uniquely identifiable on
quasi-identifiers alone**. After, 1,775 remain unique: **48.2%**.
...
99-point win. Generalising it to year gives the real 48.2%.
...
Validation runs both sides of cleaning: **3,510 failures across 2,633 rows to
...
Against 3,670 planted defects: **100% recall, 96.8% attribution** (90.0%
...
accident, now 100%. And **containment is 99.2%**: 31 rows with PII leaked into
...
**Quarantine ownership is second.** 1,316 rows per run (26.3%) set aside with
a reason is defensible engineering and an indefensible operating model with
nobody assigned. 427 fail only on a blank optional field; a tiered policy
lifts retention from 73.7% to 82.2%. Also open: no alerting threshold, and a
...
| 48.2% unique on released attributes | Assess uniqueness over everything released; generalise to a k threshold | Recital 26 |
